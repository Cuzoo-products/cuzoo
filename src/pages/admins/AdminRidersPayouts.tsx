...
import {
  parseRiderPayoutsListMeta,
  parseRiderPayoutsListPayload,
  type GetRiderPayoutsParams,
} from "@/api/admin/payouts/payoutsApi";
import AdminRiderPayoutsFilters, {
  riderPayoutsFilterFormToParams,
  riderPayoutsParamsToSearchParams,
  riderPayoutsSearchParamsToForm,
  riderPayoutsSearchParamsToParams,
  type RiderPayoutsFilterFormValues,
} from "@/components/utilities/Admins/AdminRiderPayoutsFilters";
...
export default function AdminRidersPayouts() {
  ...
  const urlFilters = useMemo(
    () => riderPayoutsSearchParamsToParams(searchParams),
    [searchParams],
  );
  const filterFormFromUrl = useMemo(
    () => riderPayoutsSearchParamsToForm(searchParams),
    [searchParams],
  );

  const [appliedFilters, setAppliedFilters] =
    useState<GetRiderPayoutsParams>(urlFilters);
  ...
  const queryParams = useMemo<GetRiderPayoutsParams>(() => {
    const params: GetRiderPayoutsParams = {
      ...appliedFilters,
      limit,
    };
  ...
  const { data, isLoading, isFetching, error } = useRidersPayouts(queryParams);

  const apiPayouts = useMemo(() => parseRiderPayoutsListPayload(data), [data]);
  const meta = useMemo(() => parseRiderPayoutsListMeta(data), [data]);
  ...
  const tableData: PayoutData[] = apiPayouts.map((p) => {
    ...
    return {
      id: recordId
        ? `/admins/payouts/riders/${encodeURIComponent(recordId)}`
        : "",
  ...
  const handleApplyFilters = (filters: RiderPayoutsFilterFormValues) => {
    const params = riderPayoutsFilterFormToParams(filters);
    setAppliedFilters(params);
    resetPagination();
    setSearchParams(riderPayoutsParamsToSearchParams(params), { replace: true });
  };
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle={
          meta?.count != null
            ? `Review rider payout requests and status · ${meta.count.toLocaleString("en-NG")} total`
            : "Review rider payout requests and status"
        }
      />

      <AdminRiderPayoutsFilters
